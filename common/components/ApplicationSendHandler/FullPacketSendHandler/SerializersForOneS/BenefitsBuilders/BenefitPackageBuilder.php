...
class BenefitPackageBuilder extends BaseApplicationPackageBuilder implements ILocalFinder
{
    public $prefs;
        ...
        foreach ($this->prefs as $pref) {
            ...
            $tmp = [
                ...
                'Document' => [
                    ...
                    'IssueDate' => date('Y-m-d', strtotime($pref->document_date) ?: strtotime(BenefitPackageBuilder::EMPTY_DATE)),
                    'DocumentCheckStatusRef' => $pref->buildDocumentCheckStatusRefType(),
                    'ReadOnly' => $pref->read_only ? 1 : 0,
                ],
                'BenefitRef' => ReferenceTypeManager::GetReference($pref, 'privilege'),
    ...
    public function update($raw_data): bool
    {
        ...
        foreach ($this->application->getBachelorPreferencesSpecialRight()->andFilterWhere(['not in', 'id', $touched_ids])->all() as $item_to_delete) {
    ...
    public function findLocalByRaw($raw_record, $excluded_ids = []): ?ActiveRecord
    {
        return BachelorPreferences::GetOrCreateFromRaw(
            ...
            ArrayHelper::getValue($raw_record, 'BenefitRef'),
            null,
