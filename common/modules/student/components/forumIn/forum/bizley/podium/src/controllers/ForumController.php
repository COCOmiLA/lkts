...
use common\modules\student\components\forumIn\forum\bizley\podium\src\models\forms\SearchForm;
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'ruleConfig' => ['class' => 'common\modules\student\components\forumIn\forum\bizley\podium\src\filters\LoginRequiredRule'],
                'rules' => [
            ...
            if (empty($model->query) && empty($model->author)) {
                ...
                if (!empty($model->query)) {
                    ...
                    foreach ($words as $word) {
                        if (mb_strlen((string)$word, 'UTF-8') > 2) {
                            $checkedWords[] = $word;
                        }
                    }
                    $model->query = implode(' ', $checkedWords);
                    if (mb_strlen((string)$model->query, 'UTF-8') < 3) {
