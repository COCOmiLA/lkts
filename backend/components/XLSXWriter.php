...
class XLSXWriter
{
    ...
    private function initializeColumnTypes($header_types)
    {
        ...
        foreach ($header_types as $v) {
            ...
            $column_types[] = array('number_format' => $number_format,
    ...
    protected function finalizeSheet($sheet_name)
    {
        ...
        $padding_length = $sheet->max_cell_tag_end - $sheet->max_cell_tag_start - strlen((string)$max_cell_tag);
    ...
    protected function styleFontIndexes()
    {
        ...
        foreach ($this->cell_styles as $i => $cell_style_string) {
            ...
            $style = json_decode((string)$style_json_string, $as_assoc = true);
            ...
            if (isset($style['border']) && is_string($style['border']))
            {
                ...
                if (isset($style['border-color']) && is_string($style['border-color']) && $style['border-color'][0] == '#') {
                    ...
                    $v = strlen((string)$v) == 3 ? $v[0] . $v[0] . $v[1] . $v[1] . $v[2] . $v[2] : $v;
    ...
    protected function writeStylesXML()
    {
        ...
        foreach ($fonts as $font) {
            if (!empty($font)) { 
                $f = json_decode((string)$font, true);
                $file->write('<font>');
        ...
        foreach ($borders as $border) {
            if (!empty($border)) { 
                $pieces = json_decode((string)$border, true);
    ...
    public static function sanitize_sheetname($sheetname)
    {
        ...
        $sheetname = trim(trim(trim((string)$sheetname), "'"));
    ...
    private static function numberFormatStandardized($num_format)
    {
        ...
        $ignore_until = '';
        $escaped = '';
        for ($i = 0, $ix = strlen((string)$num_format); $i < $ix; $i++) {
            $c = $num_format[$i];
            if ($ignore_until == '' && $c == '[')
                $ignore_until = ']';
    ...
    public static function convert_date_time($date_input) 
    {
        ...
        if ("$year-$month-$day" == '1899-12-31') return $seconds;    
        if ("$year-$month-$day" == '1900-01-00') return $seconds;    
        if ("$year-$month-$day" == '1900-02-29') return 60 + $seconds;    
